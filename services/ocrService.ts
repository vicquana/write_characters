...
const TESSERACT_OPTIONS = {
  ...
  corePath:
    "https://cdn.jsdelivr.net/npm/tesseract.js-core@5/dist/tesseract-core.wasm.js",
...
export async function extractChineseCharactersFromImage(
  image: File | Blob | string
): Promise<string[]> {
  ...
  const {
    data: { text },
  } = await tesseract.recognize(image, "chi_tra", TESSERACT_OPTIONS);

  const cleanedText = text.replace(/[\s\d\p{P}]/gu, "");
  const characters = Array.from(cleanedText).filter((char) =>
    HAN_REGEX.test(char)
  );
